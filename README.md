...
- 📞 **Телефон:** +79119947871 (предпочтительный способ связи — звоните)
- 💬 **Telegram:** @dev_ilya87 (VPN не всегда включён, поэтому лучше звонить)
- ✉️ **Email:** decilya@gmail.com, ilya.v87v@gmail.com

> Открыт к предложениям о постоянной работе и к подработке. Буду рад обсудить сотрудничество!
...
### Страница книги (просмотр)
![Просмотр книги](screenshots/book.png)
---
...
| Отписка? | Не требуется, так как в ТЗ явно указано, что это не требуется.                                                                                                                                                                                                    |
...
| RBAC? | Не используется, достаточно AccessControl. RBAC не используется, поскольку ролевая модель проста: достаточно AccessControl с разделением на гостей (`?`) и авторизованных пользователей (`@`). Внедрение полноценного RBAC было бы избыточным для данной задачи. |
...
Текущая реализация полностью закрывает требования ТЗ. Однако при росте нагрузки архитектура легко масштабируется:

1. **Асинхронная отправка SMS:**  
   Внедрить `yiisoft/yii2-queue` (драйвер Redis или DB). Метод `BookService::notifySubscribers` будет не вызывать API напрямую, а помещать `SendSmsJob` в очередь. Это полностью исключит влияние задержек внешнего SMS-шлюза на время отклика веб-сервера.

2. **Экспорт отчетов:**  
   Если в будущем потребуется функционал выгрузки отчета ТОП-10 в PDF или Excel, эту тяжелую операцию целесообразно вынести в фоновый джоб (`GenerateReportJob`). Веб-сервер будет лишь инициировать задачу, а пользователь получит уведомление или ссылку на скачивание после готовности файла.
